...
export const Header = () => {
  ...
  const currentBoardsCategory = typeof currentBoard[0].categoryId === 'number' ? categories?.filter((item) => currentBoard[0].categoryId === item.id) : null;
    ...
    <div className="bg-gray-0 text-xl-base absolute z-30 w-full px-4 py-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          {currentBoardsCategory && <p className="text-gray-60 text-m-base pt-2">{currentBoardsCategory[0].name}</p>}
        </div>
